...
class JspEnv_v1(gym.Env, ABC):
    ...
    def get_state(self):
        ...
        return deepcopy(self.env), self.env.reward
...
if __name__ == "__main__":
    ...
    if args.masking == "mask" and (args.algo != "AlphaZero") and (args.algo != "SAC"):
        if m == n == 3:
            ...
            ModelCatalog.register_custom_model(f'Dis_jsp_{args.instance_size}', TorchParametricActionsModelv14)
        else:
            if args.action_mode == "job":
    ...
    if args.algo == 'DQN':
        cfg = {
            ...
            "env_config": {
                "jsp": instance_calculator(args.instance_size),
                "reward_version": tune.grid_search(["E", "C", "D", "A"]),
                # "reward_version": "A",
                "scaling_divisor": 100,
    ...
    if args.algo == 'PPO' or args.algo == 'A3C' or args.algo == "AlphaZero" or args.algo == "A2C":
        config = dict({
            ...
            "num_workers": args.no_of_workers,  # parallelism
            "framework": 'torch',
            "rollout_fragment_length": m * n,
            "train_batch_size": m * n * 31,
